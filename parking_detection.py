...
def is_valid_parking_lot_image(image):
    ...
    std_dev = np.std(gray)
    if std_dev < 20:
        return (False, "Image has very little variation - it may be blank or a solid color")
    ...
    if edge_percentage < 0.01:
        return (False, "Image doesn't have enough detail to be a parking lot")
    
    if edge_percentage > 0.3:
        return (False, "Image has too much detail and may not be a parking lot")
    
    return (True, "Valid parking lot image")
...
def manual_count_based_on_image(image):
    result = is_valid_parking_lot_image(image)
    is_valid = result[0]
    reason = result[1]
